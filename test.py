...
def tes_ssl(args, eval_ds, model, test_method="hard_resize", pca=None):
    ...
    with torch.no_grad():
        ...
        for inputs, indices in tqdm(database_dataloader, ncols=100):
            features = model(inputs.to(args.device), return_feature=True)
...
def test_ssl(args, eval_ds, model, test_method="hard_resize", pca=None):
    """Compute features of the given dataset and compute the recalls."""
    ...
    with torch.no_grad():
        ...
        for inputs, indices in tqdm(database_dataloader, ncols=100):
            features = model.forward(inputs.to(args.device), return_feature=True)
            features = features.cpu().numpy()
            if pca != None:
                features = pca.transform(features)
            all_features[indices.numpy(), :] = features
