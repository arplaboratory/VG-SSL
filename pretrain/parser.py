...
def parse_arguments(dataset_name):
    parser = argparse.ArgumentParser(
        description="Pretrain for Benchmarking Visual Geolocalization",
    ...
    parser.add_argument(
        "--data_path", type=str, default="", help="Directory of the dataset"
    )
    parser.add_argument(
        "--train_batch_size",
        type=int,
    ...
    if dataset_name == "gldv2":
        ...
        parser.add_argument(
            "--arcface_s", type=float, default=30, help="s parameter of arcface loss"
        )
        parser.add_argument(
            "--arcface_margin", type=float, default=0.3, help="margin of arcface loss"
        )
        parser.add_argument(
            "--arcface_ls_eps",
            type=float,
            default=0.0,
            help="ls_eps of arcface loss. (label_smoothing)",
        )
    ...
    if args.dataset_name == "gldv2" and args.gldv2_csv is None:
        raise ValueError(
            "With datasets GLDv2 the csv file with images id and landmark_id must be passed using "
            "parameter --gldv2_csv"
        )
