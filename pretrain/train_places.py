...
if args.resume:
    model, optimizer, best_acc, start_epoch_num, not_improved_num = util.resume_train(
        args, model, optimizer
    )
    logging.info(
        f"Resuming from epoch {start_epoch_num} with best accuracy {best_acc:.4f}"
    )
else:
...
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    for i, (images, labels) in tqdm(
        enumerate(train_dl), total=len(train_dl), miniters=None, ncols=100
    ):
    ...
    logging.info(
        f"Training: Finished epoch {epoch_num:02d} in {str(datetime.now() - epoch_start_time)[:-7]}, "
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(
            enumerate(valid_dl), total=len(valid_dl), miniters=None, ncols=100
        ):
            images, labels = images.to(device), labels.to(device)
            batch_size = images.shape[0]
        ...
        logging.info(
            f"Validation : Finished epoch {epoch_num:02d} in {str(datetime.now() - epoch_start_time)[:-7]}, "
    ...
    if is_best:
        logging.info(
            f"Improved: previous best accuracy = {best_acc:.4f}, current accuracy = {valid_acc.avg:.4f}"
        )
