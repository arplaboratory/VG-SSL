...
def create_h5_file(args, resize_option=False):
    ...
    with h5py.File(args.save_path, 'a') as hf:
        ...
        for img_name in tqdm(img_names):
            img_path = os.path.join(args.read_path, img_name[0], img_name[1], img_name[2], img_name + '.jpg')
            ...
            if not start:
                if args.compress:
                    hf.create_dataset('image_data', data=img_np, chunks=True, maxshape=(None, None, None, 3), compression='lzf')  # write the data to hdf5 file
                    hf.create_dataset('image_size', data=size_np, chunks=True, maxshape=(None, 2), compression='lzf')
                else:
                    hf.create_dataset('image_data', data=img_np, chunks=True, maxshape=(None, None, None, 3))  # write the data to hdf5 file
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, help='The path of the folder that stores h5 files')
    ...
    try:
        ...
        print(f'{e}')
        print(f'Try resizing all images to ({args.resize_width}, {args.resize_height}) (WxH)')
        create_h5_file(args, resize_option=True)